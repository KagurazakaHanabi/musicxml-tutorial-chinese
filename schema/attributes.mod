...
<!--
	attributes DTD 模块包含 attributes 元素及其子元素，例如拍号和调号。
-->

<!-- Entities -->

<!--
	time-separator 实体表示如何显示 beats 和 beat-type 的值在拍号中的布局。
	
	默认值是 none。horizontal，vertical 和 diagonal 表示水平线，垂直线和从左下到右
	上的对角线。对于这些值，beats 和 beat-type 排列在分隔线的两侧。
	none 值代表 beats 和 beat-type 之间没有分隔符并垂直排列。
	adjacent 值代表 beats 和 beat-type 之间没有分隔符并水平排列。
-->
...
<!--
	音符的时值通常用分数表示。divisions 元素表示每四分音符多少个用于指示时值的分度。
	例如，如果 duration = 1，divisions = 2，这是个八分音符。duration 和 divisions 
	直接用于产生声音输出，因此必须选择它们以考虑连音。使用 divisions 元素可以让我们仅
	使用一个数字来表示乐谱中每个音符的持续时间，同时保留分数表示的全部功效。为了最大限
	度地与标准 MIDI 文件兼容，divisions 值不得超出 16383。
-->
<!ELEMENT divisions (#PCDATA)>

<!--
	拍号使用两个元素表示，bests 元素表示节拍数，是拍号的分子，beat-type 元素表示单
	位，是拍号的分母。
	...
	interchangeable 元素表示一对可互换的拍号中的第二个，例如 3/4 (6/8) 中的 6/8。
	...
	senza-misura 明确指示不存在任何拍号。可选的元素内容表示将要使用的符号，例如 X。
	存在 senza-misura 元素时，不使用 time 元素的 symbol 属性。

	print-object 属性允许指定但不打印时间签名，as is the case for excerpts
...
<!--
	tuning-step，tuning-alter 和 tuning-octave 元素在 common.mod 文件中定义。
	谱线从下往上编号。
-->
...
<!--
	capo 元素指示将变调夹放在有品乐器中的哪个品格上。这将使用指定的半音数对 
	staff-tuning 指定的开放调弦作修改。
-->
<!ELEMENT capo (#PCDATA)>
